...
ggplot(data, aes(production_year)) +
  geom_histogram(binwidth = 1, alpha=.5, position="identity", fill ="#377EB8") +
...
popularity_2014 <- "SELECT t.id, t.title, t.production_year, mi1.info AS Genres, mi2.info AS Rating
FROM title t
JOIN movie_info AS mi1 ON mi1.movie_id = t.id
...
popularity_1950 <- "SELECT t.id, t.title, t.production_year, mi1.info AS Genres, mi2.info AS Rating
FROM title t
JOIN movie_info AS mi1 ON mi1.movie_id = t.id
JOIN movie_info_idx AS mi2 ON mi2.movie_id = t.id
WHERE production_year IS NOT NULL
